...
export function ServiceCardPresenter({
  ...
  if (viewMode === 'list') {
    return (
      <div
        className={cn(
          'flex flex-col lg:flex-row gap-6 items-start w-full p-6 bg-white rounded-xl border transition-all duration-300',
          isSelected
            ? 'ring-2 ring-magenta'
            : 'border-rose-powder/30 hover:border-magenta/30',
          className
        )}
      >
        ...
        <div className="flex-1 min-w-0">
          <header className="mb-4">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
              ...
              >
                <h3 className="font-playfair text-xl lg:text-2xl text-charcoal group-hover:text-magenta">
                  {service.name}
                </h3>
                <ExternalLink className="w-4 h-4 text-charcoal/40 group-hover:text-magenta" />
              </Link>
              <div className="flex items-center gap-3">
                <span className="font-bold text-magenta text-xl lg:text-2xl">
    ...
    >
      <CardHeader className="pb-4">
        ...
        <Link href={`/service/${service.id}`} className="group">
          <CardTitle className="font-playfair text-lg text-charcoal group-hover:text-magenta">
            {service.name}
          </CardTitle>
        </Link>
      ...
      <CardContent className="flex-grow py-0">
        <CardDescription className="text-charcoal/70 line-clamp-3">
          {service.description}
        </CardDescription>
